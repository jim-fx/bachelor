...
Hierbei garantiert WebAssembly  Plattformunabhängigkeit, hohe Performance und Sicherheit. \cite{Haas2017}
\br
...
Um die VPL nach den drei Anforderungen \hyperref[sec:Zielsetzung]{Zielsetzung} zu entwickeln, muss die Architektur so gestaltet sein, dass sie einfach erweiterbar, performant und einfach zu bedienen ist.
Aus diesen Anforderungen und dem Konzept ergibt sich folgende dreiteilige Architektur:
\br
...
Nun legt man die Node-Definition an, dafür wird die Date \topic{src/definition.json} angepasst. Hier ist die Definition für die Zylinder-Node:
...
Nun wird die Implementierung in der Datei \topic{src/lib.rs} angepasst. Der \link[code_cylinder_node]{Beispielcode} legt deutlich das wenig Boilerplate-Code benötigt wird. Ein Großteil der Implementierung lag darin den Code zu schreiben der die Zylinder-Geometrie generiert.  Außerdem kann das gesamte Rust-Ökosystem genutzt werden, so zum Beispiel \href{https://docs.rs/glam/latest/glam}{glam}, eine Bibliothek für Vektoren und Matrizen.
...
Die Implementierung der Anwendung hat gezeigt, dass die Vorteile die WebAssembly bietet die \link[HON]{Komplexität der Implementierung}, die durch die Nachteile entstehen weit überwiegen.
...
Wenn das WebAssembly Component Model in Browsern verfügbar wird, könnte man den \link[runtime_executor]{Runtime-Executor} als einzelnen WebAssembly Component implementieren. Dieser könnte nun \link[node]{Nodes} als WebAssembly Module laden und ausführen so könnte sich der Overhead vermeiden der durch die Kommunikation zwischen Javascript und WebAssembly entsteht.
...
use nodarium_utils::{
    geometry::{create_geometry_data, wrap_geometry_data},
};
...
pub fn execute(input: &[i32]) -> Vec<i32> {
    let arguments = split_args(input);

    let height = evaluate_float(arguments[0]);
    let radius = evaluate_float(arguments[1]);

    let mut geometry_data = create_geometry_data(16, 16);
    ...
    for i in 0..8 {
        ...
        // ... implementierung der cylinder node

    }

    // nun werden die daten in das serialisierte Binärformat umgewandelt
    let mut output: Vec<i32> = Vec::with_capacity(geometry_data.len() + 4);
    output.push(0);
    output.push(1);
    output.extend_from_slice(&geometry_data);
    output.push(1);
    output.push(1);

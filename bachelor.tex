...
Allerdings Javascript ist \qt{garbage-collected} und dynamischen typisiert und daher nicht die beste Wahl für die Entwicklung von performantem Code im Browser. Des Weiteren benötigt Javascript besondere Aufmerksamkeit, um sicherzustellen, das der Code von anderen Nutzer/innen sicher ausgeführt werden kann. 
\br
...
Nun wird die Implementierung in der Datei \topic{src/lib.rs} angepasst. Der \link[code_cylinder_node]{Beispiel-Code} legt deutlich das wenig Boilerplate-Code benötigt wird. Ein Großteil der Implementierung lag darin den Code zu schreiben der die Zylinder-Geometrie generiert.  Außerdem kann das gesamte Rust-Ökosystem genutzt werden, so zum Beispiel \href{https://docs.rs/glam/latest/glam}{glam}, eine Bibliothek für Vektoren und Matrizen.

...
\br
Die einzelnen Argumente können entweder direkt über das Interface einer \link{Node} definiert werden oder in dem der/die Nutzer/in das Ergebnis einer anderen \link{Node} mit diesem Argument verknüpft.
...
\br
Die Verbindungen mehrerer \link[Node]{Nodes} stellt einen \link{Node-Graph} dar. 
Dieser \link{Node-Graph} kann als JSON-Objekt serialisiert und deserialisiert werden und so gespeichert und geladen werden.
...
Vorstellbar wäre zum Beispiel das der/die Nutzer/in den \link{Node-Graph} in einer Web-App bearbeitet, dieser aber in Echtzeit auf einem Server ausgeführt wird.

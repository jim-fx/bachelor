...
Das \link[node_interface]{NODE\_INTERFACE} ist für das visuelle Interface zuständig, hier interagieren Nutzer/innen mit dem \link[node_graph]{Node-Graph}.
Der \link[runtime_executor]{RUNTIME\_EXECUTOR} ist für die Ausführung der Nodes zuständig. Er nimmt einen serialisierten \link[node_graph]{Node-Graph} entgegen und gibt das Ergebnis zurück.
Die \link[node_registry]{NODE\_REGISTRY} ist für das Verwalten der Nodes zuständig. Die beiden anderen Komponenten laden aus ihr die einzelnen WebAssembly-Binärdateien der Nodes.
\br
...
Diese Inputs legen fest welche \link[node]{Nodes} mit dieser \link[node]{Node} verbunden werden können. 
Außerdem benutzt das \link[node_interface]{Node-Interface} diese Information, um das Interface einer Node zu konstruieren.
\br
...
Die \link[node]{Node} Datenstruktur besteht aus einer \topic{id}, einem \topic{type}, optionalen \topic{props} und einer \topic{position}. 
